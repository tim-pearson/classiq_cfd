...
For example, in finance the state may represent the price distribution of some
assets.
In chemistry, it may be an initial guess for the ground state of a molecule,
and in
...
The probabilities are a list of positive numbers. This is the resulting wave
...
by an error bound, using the [$L_2$
...
- `probabilities: CArray[CReal]` - Probabilities to load. Should be
non-negative and sum to 1.
- `bound: CReal` - Approximation Error Bound, in the $L_2$ metric (with respect
to the given probabilies vector).
...
- `amplitudes: CArray[CReal]` - Amplitudes of the loaded state. Each should be
real and the vector norm should be equal to 1.
- `bound: CReal` - Approximation Error Bound, in the $L_2$ metric (with respect
to the given amplitudes vector).
- `out: Output[QArray[QBit]]`
...
The `inplace_prepare_amplitudes` works the same, but for a given allocated
`QArray`."""
...
This example generates a quantum program whose output state probabilities are
an approximation to the PMF given.
That is, the probability of measuring the state $|000⟩$ is $0.05$, $|001⟩$ is
$0.11$,...
